...
** Usage
   ...
   For a heading to be exported as a host, it must have its heading equal to the global identifier
   or either a =HOSTNAME= or =IP= property. If an entry has both, =IP= takes precedence. It can also
   contain one or more optional parameters, listed in the table below.
